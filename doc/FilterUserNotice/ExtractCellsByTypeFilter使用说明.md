...
3. 左侧功能栏列出该模型所有单元类型勾选框（默认全选）；**此时尚未生成任何新模型**；
4. 勾选需要的类型 → 点【提取】→ 模型树新增 $\text{ExtractCellsByType}_1$，**原模型保留不动**；
5. 再次修改勾选 → 点【提取】→ $\text{ExtractCellsByType}_1$ 原地更新（不新增节点）；
6. 再次使用菜单 → 生成 $\text{ExtractCellsByType}_2$，互不干扰。
...
| 注意点 | 说明 |
|--------|------|
| **打开面板不自动执行** | 打开左侧面板只列出可提取的单元类型（默认全选），**不会**自动生成模型；必须点【提取】后才生成 $\text{ExtractCellsByType}_n$，避免一打开面板就产生一个与原模型内容相同的重复模型 |

...
该 Filter 不生成新的几何模型，而是在输入数据对象上添加坐标属性。属性数组与模型的点坐标共享数据，因此点坐标发生修改后，再次执行 Filter 即可更新数组值及其数据范围。

## 2. 输入与输出

- 输入：含有点坐标的 `DataObject`，例如 `UnstructuredMesh`、`SurfaceMesh` 或 `PointSet`。
- 输出：与输入相同的数据对象，并新增一个三分量、点关联的向量数组。
- 默认数组名：`Coordinates`。
- 数组元组数：与输入模型点数一致。
...
示例自动检查：模型读取、坐标值及三分量结构、点关联方式、拓扑不变、重复执行后的实时更新、空数据和同名数组冲突处理。

## 6. 注意事项

- 输入对象必须具有点坐标；空点集允许生成空的三分量数组，但完全没有 `Points` 的对象会执行失败。
- 数组名不能为空。调用 `SetArrayName()` 可修改默认名称。
- 若输入中已经存在同名但不是当前坐标存储的属性，Filter 会失败并保留原属性，避免静默覆盖数据。
- Filter 会修改输入对象本身，输出指针与输入指针相同；依赖不可变输入的流程应提前复制数据。
